...
def exit():
    ...
    cart_items = db.execute("SELECT * FROM cart WHERE user_id = :user_id", user_id=session['user_id'])
    
    if cart_items:
        # Process the checkout and create an order
        order_id = db.execute("INSERT INTO orders (user_id, date) VALUES (:user_id, :date)",
                              user_id=session['user_id'], date=datetime.now())
        
        for item in cart_items:
            db.execute("INSERT INTO order_items (order_id, product_id, quantity) VALUES (:order_id, :product_id, :quantity)",
        ...
        # Display the checkout page if no items in cart
        return render_template("exit.html", cart_items=cart_items)
...
def order_complete():
    ...
    # Process the order (e.g., save to database, clear cart)
    # Retrieve user cart
    cart_items = db.execute("SELECT * FROM cart WHERE user_id = :user_id", user_id=session["user_id"])
    if not cart_items:
    ...
    for item in cart_items:
        db.execute("INSERT INTO order_items (order_id, product_id, quantity) VALUES (:order_id, :product_id, :quantity)",
                   order_id=order_id, product_id=item["id"], quantity=item["qty"])

    # Clear the cart
    db.execute("DELETE FROM cart WHERE user_id = :user_id", user_id=session["usser_id"])
